...
// _conv 函数保持不变，因为它已经使用了动态内存分配的参数
...
void _statistics(int N, float *flux, float* mag, float *magerr, float* time, float *mask,
  ...
  // 动态分配内存
  float *rateG1 = malloc(N * sizeof(float));
  float *rateG2 = malloc(N * sizeof(float));
  float *maskG1 = malloc(N * sizeof(float));
  float *maskG2 = malloc(N * sizeof(float));
  if (!rateG1 || !rateG2 || !maskG1 || !maskG2) {
      // 如果内存分配失败，处理错误
